...
void APCDriver::loop(USBHostUPS* host, UPSData& data, uint32_t now) {
    ...
    if (now - _last_fast_poll >= 2000 || _last_fast_poll == 0) {
        ...
        host->requestReport(0x0B, 0x03, 2); // APC/CyberPower status
    ...
    if (_poll_step > 0) {
        if (now - _last_step_time >= 50 || _poll_step == 1) {
            ...
            switch (_poll_step) {
                ...
                case 4: host->requestReport(0x10, 0x03, 2); break; // High Voltage Transfer
                case 5: host->requestReport(0x18, 0x03, 2); break; // Config Active Power
...
void APCDriver::decodeReport(USBHostUPS* host, uint8_t report_id, const uint8_t *data, size_t length, UPSData& ups_data) {
    ...
    switch (report_id) {
        ...
        case 0x07: // Load
            if (length - offset >= 6) { // To support Eaton-like format dove il carico è offset 5
