...
void test_decode_battery_charge(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(85, test_usb_ups.getUPSData()->getFloat("battery.charge"));
    ...
    TEST_ASSERT_EQUAL_UINT8(92, test_usb_ups.getUPSData()->getFloat("battery.charge"));
}
...
void test_suppress_chrg_at_100(void) {
    ...
    test_usb_ups._ups_data.set("ups.status.ac_present", "1");
    test_usb_ups._ups_data.set("ups.status.discharging", "0");
    test_usb_ups._ups_data.set("ups.status.charging", "1");
    
    // Test 1: < 100% -> should have CHRG
    test_usb_ups._ups_data.set("battery.charge", "99");
    TEST_ASSERT_EQUAL_STRING("OL CHRG", test_usb_ups.getUPSStatusString().c_str());
    
    // Test 2: == 100% -> should NOT have CHRG
    test_usb_ups._ups_data.set("battery.charge", "100");
    TEST_ASSERT_EQUAL_STRING("OL", test_usb_ups.getUPSStatusString().c_str());
}
...
void test_decode_input_voltage(void) {
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 230.0f, test_usb_ups.getUPSData()->getFloat("output.voltage"));
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 225.5f, test_usb_ups.getUPSData()->getFloat("output.voltage"));
}
...
void test_dev_gone(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0, test_usb_ups.getUPSData()->getFloat("battery.charge"));
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 0.0f, test_usb_ups.getUPSData()->getFloat("output.voltage"));
}
...
void test_decode_string_descriptor(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("EATON", test_usb_ups.getUPSData()->get("ups.mfr").c_str());
    ...
    TEST_ASSERT_EQUAL_STRING("3S UPS", test_usb_ups.getUPSData()->get("ups.model").c_str());

    // Empty string
    uint8_t data_empty[] = {2, 0x03};
    test_driver.parseStringDescriptor(4, data_empty, sizeof(data_empty), test_usb_ups._ups_data);
    TEST_ASSERT_EQUAL_STRING("", test_usb_ups.getUPSData()->get("ups.serial").c_str());
}
...
void test_decode_battery_chemistry(void) {
    ...
    test_usb_ups._ups_data.set("battery.type", "");
    ...
    TEST_ASSERT_EQUAL_STRING("PbAc", test_usb_ups._ups_data.get("battery.type").c_str());
